 ...
 */
public class
OdtInputConverter implements InputConverter {
    /**
    ...
    public byte[] getPersonalizedTemplate(Paper paper, SubmissionLanguage submissionLanguage)
        ...
        injectAbstractIntoContent(contentDoc, paper.getSubmissionAbstract());
    ...
    public void injectAbstractIntoContent(Document contentDoc, String submissionAbstract) throws IOException {
        Nodes searchResult =
                contentDoc.query("//text:section[@text:name='AuthorsInfoPost']", xPathContext);

        if (!(searchResult.get(0) instanceof Element)) {
            throw new IllegalStateException(Messages.getString("OdtInputConverter.authorsinfoposterror"));
        }

        Element authorsInfoPostSection = (Element) searchResult.get(0);
        ParentNode parentNode = authorsInfoPostSection.getParent();
        int mainBodyIndex = parentNode.indexOf(authorsInfoPostSection) + 1;

        parentNode.removeChild(mainBodyIndex);

        Element abstractSectionElement = new Element("p", Namespace.TEXT.toUri());
        parentNode.insertChild(abstractSectionElement, mainBodyIndex);

        Element abstractHeadingElement = new Element("p", Namespace.TEXT.toUri());
        parentNode.insertChild(abstractHeadingElement, mainBodyIndex + 1);
        abstractHeadingElement.appendChild("Abstract");
        abstractHeadingElement.addAttribute(
                new Attribute("text:style-name", Namespace.TEXT.toUri(), "DH-Heading1")
        );

        try {
            Document abstractDoc = DOMConverter.convert(
                    DocumentBuilderFactory.newInstance()
                            .newDocumentBuilder()
                            .parse(new InputSource(new StringReader("<xml>" + submissionAbstract + "</xml>")))
            );

            for (int i = 0; i < abstractDoc.getChildCount(); i++) {
                parentNode.insertChild(
                        new Element("p", Namespace.TEXT.toUri()), mainBodyIndex + i + 2
                );
                Element abstractContentElement = (Element) parentNode.getChild(mainBodyIndex + i + 2);
                abstractContentElement.appendChild(abstractDoc.getChild(i).getValue());
                abstractContentElement.addAttribute(
                        new Attribute("text:style-name", Namespace.TEXT.toUri(), "DH-Default")
                );
            }
